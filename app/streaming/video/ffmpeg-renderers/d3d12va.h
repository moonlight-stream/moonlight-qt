...
private:
    ...
    struct VppSurface {
        ...
        mfxSurfaceD3D11Tex2D    textureLinkD3D11;
        mfxSurfaceD3D12Tex2D    textureLinkD3D12;
        ComPtr<ID3D11Texture2D> textureD3D11;
        ComPtr<ID3D12Resource>  textureD3D12;
    };
    
    static void lockContext(void* lock_ctx);
    static void unlockContext(void* lock_ctx);
    
    // The frame color space is SMPTE2084 (HDR content)
    static bool m_IsFrameHDR;
    // The client display HDR status is On
    static bool m_IsClientHDR;
    // The frame texture is 10-bits (HDR and SDR)
    bool m_IsTexture10bits = false;
    // The frame texture is YUV 4:4:4
    bool m_IsTextureYUV444 = false;
    // The Decoder is providing a D3D11 frame
    bool m_IsFrameD3D11 = false;

    bool verifyHResult(HRESULT hr, const char* operation);
    bool checkDecoderDX11Support();
    bool checkDecoderDX12Support();
    ...
    std::string m_InfoUpscaler = "None";
    std::string m_InfoSharpener = "None";
    std::string m_InfoAlgo = "None";
    ...
    QFuture<void> m_HDRUpdateFuture = QFuture<void>();
    ...
    // Overlay
    std::atomic<bool> m_OverlaySkip = false;
    ...
    ComPtr<ID3D11VideoDevice2> m_D3D11VideoDevice;
    ComPtr<ID3D11DeviceContext4> m_D3D11DeviceContext;
    ComPtr<ID3D11Texture2D> m_D3D11FrameTexture; // Input AMF
    ComPtr<ID3D11Texture2D> m_D3D11YUVTextureUpscaled; // Output VPL
    ...
    mfxLoader m_VppLoader;
    mfxFrameAllocRequest m_VppReq[2];
