...
bool D3D12VARenderer::m_IsFrameG2084 = false;
 ...
 *
 * \params bool isFrameG2084
 * \return void
 */
void D3D12VARenderer::updateDisplayHDRStatusAsync(bool isFrameG2084)
{    
    // Swapchain ColorSpace can be changed in live
    if (isFrameG2084 != D3D12VARenderer::m_IsFrameG2084) {
        D3D12VARenderer::m_IsFrameG2084 = isFrameG2084;
        ...
        m_RGBColorSpace = (D3D12VARenderer::m_IsFrameG2084 || m_VendorHDRenabled) ? DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020 : DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709; 
...
void D3D12VARenderer::enhanceAutoSelection()
{
    ...
    if(D3D12VARenderer::m_IsFrameG2084 || !D3D12VARenderer::m_IsClientHDR){
        m_VendorHDRenabled = false;
    }
...
bool D3D12VARenderer::enableAMDVideoSuperResolution(bool activate, bool logInfo)
{
    ...
    if(m_AmfHdrColorSpaceEnabled && D3D12VARenderer::m_IsFrameG2084){
...
{
    if(!m_AmfInitialized || !m_AmfHdrColorSpaceEnabled || !D3D12VARenderer::m_IsFrameG2084)
        return;
...
{
    return D3D12VARenderer::m_IsFrameG2084 ? COLORSPACE_REC_2020 : COLORSPACE_REC_709;
}
    ...
    // Variables initalization
    {
        ...
        if (!m_DecoderParams.testOnly) {
            ...
            do {
                ...
                if (SDL_GetWindowDisplayMode(m_DecoderParams.window, &windowMode) != 0)
                    break;
                
                // The application setting
                SDL_DisplayMode streamMode = windowMode;
                streamMode.w = m_DecoderParams.width;
                streamMode.h = m_DecoderParams.height;
                streamMode.refresh_rate = m_DecoderParams.frameRate;
                ...
                // Only in exclusive mode, if the application is targeting higher resolution, we using the application selected resolution
                if (fullscreenExclusive && streamMode.h > desktopMode.h) {
                    // Check if the display propose this resolution
                    index = 0;
                    while (EnumDisplaySettingsExW(m_DisplayDevice.DeviceName, index, &dmTemp, 0)) {
                        // Only check setups with the same resolution
                        if (dmTemp.dmPelsWidth == streamMode.w && dmTemp.dmPelsHeight == streamMode.h) {
                ...
                // NOTE:
                // Windowed + Borderless: Keep display resolution / Keep display frequency
                // Windowed + Borderless + vSync: Keep display resolution / set closest Set highest frequency (to get use of VRR)
                // Exclusive: Set targeted resolution / Set highest frequency (to reduce tearing)
                // Exclusive + vSync: Set targeted resolution / Set highest frequency (to get use of VRR)
                ...
                if (fullscreenExclusive
                    || targetMode.refresh_rate != m_DevMode.dmDisplayFrequency
                    || targetMode.w != windowMode.w
                    || targetMode.h != windowMode.h) {
                    ...
                    if (targetMode.w != windowMode.w || targetMode.h != windowMode.h) {
                        m_DevMode.dmFields |= DM_PELSWIDTH | DM_PELSHEIGHT;
                        m_DevMode.dmPelsWidth = targetMode.w;
                        m_DevMode.dmPelsHeight = targetMode.h;
                    }
        ...
        if (D3D12VARenderer::m_IsFrameG2084) {
