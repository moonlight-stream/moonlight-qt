...
D3D12VARenderer::~D3D12VARenderer()
{
    ...
    if(m_DebugLayer && m_DebugVerbose){
...
bool D3D12VARenderer::checkDecoderType()
{
    ...
    // NVIDIA GPUs always support well the decoding in DX12
    else if (m_VideoEnhancement->isVendorNVIDIA()) {
        return !m_IsFrameD3D11;
    }

    // FFmpeg for Snapdragon GPU only works with DX11
    else if (m_VideoEnhancement->isVendorQualcomm()) {
        return m_IsFrameD3D11;
    }
...
{    
    ...
    UINT dxgiFactoryFlags = m_DebugLayer ? DXGI_CREATE_FACTORY_DEBUG : 0;
    ...
    if (FAILED(m_hr)) {
        ...
        m_hr = CreateDXGIFactory2(0, __uuidof(IDXGIFactory6), (void**)m_Factory.GetAddressOf());
        if(!verifyHResult(m_hr, "CreateDXGIFactory2(0, __uuidof(IDXGIFactory6), (void**)m_Factory.GetAddressOf());")){
            return false;
        }
    }
    ...
    {   
        // D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS1 inputStreamFiltersArgs = {}; // Unused
        ...
        if(m_VideoProcessorConvertEnabled){
            ...
            if(!verifyHResult(m_hr, "m_VideoDevice->CreateVideoProcessor1(... m_VideoProcessorConvert)")){
                m_VideoProcessorConvertEnabled = false;
            }
        }
        ...
        if(m_VideoProcessorUpscalerEnabled){
            ...
            if(!verifyHResult(m_hr, "m_VideoDevice->CreateVideoProcessor1(... m_VideoProcessorUpscaler)")){
                m_VideoProcessorUpscalerEnabled = false;
            }
        }
        ...
        if(m_VideoProcessorUpscalerConvertEnabled){
            ...
            if(!verifyHResult(m_hr, "m_VideoDevice->CreateVideoProcessor1(... m_VideoProcessorUpscalerConvert)")){
                m_VideoProcessorUpscalerConvertEnabled = false;
            }
        }
    }
    ...
    // Use the corresponding decoder (FFmpeg)
    {
        if (m_IsFrameD3D11) {
            ...
            d3d12vaDeviceContext->lock = [](void* ctx){
                Q_UNUSED(ctx);
            }; // Do nothing
            d3d12vaDeviceContext->unlock = [](void* ctx){
                Q_UNUSED(ctx);
            }; // Do nothing
