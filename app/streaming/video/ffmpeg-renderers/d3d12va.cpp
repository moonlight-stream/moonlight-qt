...
D3D12VARenderer::~D3D12VARenderer()
{
    ...
    m_GlobalCommandList.Reset();
    m_GlobalCommandQueue.Reset();
...
bool D3D12VARenderer::initialize(PDECODER_PARAMETERS params)
{
    ...
    {
        // GLOBAL
        // Note: This queue does nothing, it is there only for the "wait GPU" process,
        // it helps to not block the CPU
        m_hr = m_Device->CreateCommandAllocator(
            D3D12_COMMAND_LIST_TYPE_DIRECT,
            IID_PPV_ARGS(&m_GlobalCommandAllocator)
            );
        if(!verifyHResult(m_hr, "m_Device->CreateCommandAllocator(... m_GlobalCommandAllocator)")){
            return false;
        }
        ...
        m_hr = m_Device->CreateCommandList(
            0,
            D3D12_COMMAND_LIST_TYPE_DIRECT,
            m_GlobalCommandAllocator.Get(),
            nullptr,
            IID_PPV_ARGS(&m_GlobalCommandList)
            );
        if(!verifyHResult(m_hr, "m_Device->CreateCommandList(... m_GlobalCommandList)")){
            return false;
        }
        
        D3D12_COMMAND_QUEUE_DESC queueGlobalDesc = {};
        queueGlobalDesc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT;
        queueGlobalDesc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_NORMAL;
        queueGlobalDesc.Flags = D3D12_COMMAND_QUEUE_FLAG_NONE;
        queueGlobalDesc.NodeMask = 0;
        m_hr = m_Device->CreateCommandQueue(&queueGlobalDesc, IID_PPV_ARGS(&m_GlobalCommandQueue));
        if(!verifyHResult(m_hr, "m_Device->CreateCommandQueue(&queueGlobalDesc, IID_PPV_ARGS(&m_GlobalCommandQueue));")){
            return false;
        }
 ...
 * \return void
 */
void D3D12VARenderer::waitForDecoder(bool waitCPU)
{
    ...
    if (!waitCPU) {
        m_GlobalCommandQueue->Wait(m_FenceDecoder.Get(), m_FenceDecoderValue);
    } else if (m_FenceDecoder->GetCompletedValue() < fence) {
...
void D3D12VARenderer::waitForVideoProcess(bool waitCPU)
{
    ...
    if (!waitCPU) {
        m_GlobalCommandQueue->Wait(m_FenceVideoProcess.Get(), m_FenceVideoProcessValue);
...
void D3D12VARenderer::waitForGraphics(bool waitCPU)
{
    ...
    if (!waitCPU) {
        m_GlobalCommandQueue->Wait(m_FenceGraphics.Get(), m_FenceGraphicsValue);
...
void D3D12VARenderer::waitForOverlay(bool waitCPU)
{
    ...
    if (!waitCPU) {
        m_GlobalCommandQueue->Wait(m_FenceOverlay.Get(), m_FenceOverlayValue);
...
void D3D12VARenderer::renderFrame(AVFrame* frame)
{
    ...
    if(m_VideoEnhancement->getDeviceType() == AV_HWDEVICE_TYPE_D3D11VA){
        ...
        m_GlobalCommandQueue->Wait(m_D3D12Fence.Get(), m_D3D11FenceValue);
