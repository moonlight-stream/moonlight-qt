...
bool CoreAudioRenderer::prepareForPlayback(const OPUS_MULTISTREAM_CONFIGURATION* opusConfig)
{
    ...
    // Set channel layout on Output AudioUnit for spatial audio mode.
    // This may help macOS recognize spatial audio usage, though the indicator
    // typically only appears with AVAudioEngine. Spatial audio processing
    // works correctly regardless of indicator state.
    if (m_Spatial && outputType == kSpatialMixerOutputType_Headphones) {
#if TARGET_OS_OSX
        AudioChannelLayout stereoLayout = {};
        stereoLayout.mChannelLayoutTag = kAudioChannelLayoutTag_Stereo;
        stereoLayout.mChannelBitmap = 0;
        stereoLayout.mNumberChannelDescriptions = 0;

        UInt32 layoutSize = offsetof(AudioChannelLayout, mChannelDescriptions);
        status = AudioUnitSetProperty(m_OutputAU, kAudioUnitProperty_AudioChannelLayout,
                                     kAudioUnitScope_Input, 0, &stereoLayout, layoutSize);
        if (status != noErr) {
            CA_LogError(status, "Failed to set Output AudioUnit channel layout for spatial audio");
            // Non-fatal: optional for functionality, may help system recognition
        } else {
            DEBUG_TRACE("CoreAudioRenderer set Output AudioUnit channel layout for spatial audio");
        }
#endif
    }
